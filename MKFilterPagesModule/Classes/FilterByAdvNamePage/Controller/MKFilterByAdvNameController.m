...
@interface MKFilterByAdvNameController ()<UITableViewDelegate,
...
@implementation MKFilterByAdvNameController

- (void)dealloc {
    NSLog(@"MKFilterByAdvNameController销毁");
}
...
- (void)loadSection0Datas {
    ...
    cellModel1.isOn = self.protocol.match;
    [self.section0List addObject:cellModel1];
    
    MKTextSwitchCellModel *cellModel2 = [[MKTextSwitchCellModel alloc] init];
    cellModel2.index = 1;
    cellModel2.msg = @"Reverse Filter";
    cellModel2.isOn = self.protocol.filter;
    [self.section0List addObject:cellModel2];
}

- (void)loadSection1Datas {
    for (NSInteger i = 0; i < self.protocol.nameList.count; i ++) {
        MKTextFieldCellModel *cellModel = [[MKTextFieldCellModel alloc] init];
        cellModel.index = i;
        cellModel.msg = [NSString stringWithFormat:@"ADV Name %ld",(long)(i + 1)];
        cellModel.textPlaceholder = @"1-20 Characters";
        cellModel.textFieldType = mk_normal;
        cellModel.textFieldValue = self.protocol.nameList[i];
        cellModel.maxLength = 20;
        [self.section1List addObject:cellModel];
    }
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKFilterPagesModule", @"MKFilterByAdvNameController", @"mk_filterPages_slotSaveIcon.png") forState:UIControlStateNormal];
