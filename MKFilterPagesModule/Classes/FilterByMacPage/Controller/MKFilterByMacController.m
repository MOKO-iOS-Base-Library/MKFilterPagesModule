...
@interface MKFilterByMacController ()<UITableViewDelegate,
...
@implementation MKFilterByMacController

- (void)dealloc {
    NSLog(@"MKFilterByMacController销毁");
}
...
- (void)loadSection1Datas {
    for (NSInteger i = 0; i < self.protocol.macList.count; i ++) {
        MKTextFieldCellModel *cellModel = [[MKTextFieldCellModel alloc] init];
        cellModel.index = i;
        cellModel.msg = [NSString stringWithFormat:@"MAC %ld",(long)(i + 1)];
        cellModel.textPlaceholder = @"1-6 Bytes";
        cellModel.textFieldType = mk_hexCharOnly;
        cellModel.textFieldValue = self.protocol.macList[i];
        cellModel.maxLength = 12;
        [self.section1List addObject:cellModel];
    }
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKFilterPagesModule", @"MKFilterByMacController", @"mk_filterPages_slotSaveIcon.png") forState:UIControlStateNormal];
